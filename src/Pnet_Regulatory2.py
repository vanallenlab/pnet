...
import util
import ReactomeNetwork_2Regulatory
...
class PNET_NN(pl.LightningModule):
    def __init__(self, reactome_network, task, nbr_gene_inputs=1, output_dim=1, additional_dims=0, lr=1e-3, weight_decay=1e-5,
        ...
        gene_masks, pathway_masks, input_mask, extra_mask = self.reactome_network.get_masks(self.nbr_gene_inputs)
        self.extra_mask = extra_mask.values 
        self.extra_genes = extra_mask.columns
        if random_network:
        ...
        self.extra_layer = nn.Sequential(*masked_activation(self.extra_mask, activation=self.activation, batchnorm=True, dropout=self.gene_dropout))
        self.preds.append(
                nn.Sequential(*[nn.Linear(in_features=self.extra_mask.shape[1] + self.additional_dims,
    ...
    def forward(self, x, additional_data):
        ...
        x_reg = self.extra_layer(x)
        x_cat = torch.concat([x_reg, additional_data], dim=1)
        y_hats.append(self.preds[0](x_cat))
        
        
        x = self.first_gene_layer(x + x_reg)
        x_cat = torch.concat([x, additional_data], dim=1)
        y_hats.append(self.preds[1](x_cat))
        
        
        
        
        for layer, pred in zip(self.layers, self.preds[2:]):
            x = layer(x, genes)
            x_cat = torch.concat([x, additional_data], dim=1)
    ...
    """
    def extra_layer_importance(self, test_dataset, target_class=0):
        self.interpret_flag=True
        
        act = captum.attr.LayerActivation(self, self.extra_layer)
        act_vals = act.attribute((test_dataset.x, test_dataset.additional), attribute_to_layer_input=True)
        cols = self.extra_genes
        act_vals_genomic = pd.DataFrame(act_vals.detach().numpy(),
                                            columns=cols,
                                            index=test_dataset.input_df.index)
        layer_imp_by_target = act_vals_genomic
        self.interpret_flag=False
        return layer_imp_by_target
    
    """
    def extra_layer_importance(self, test_dataset, target_class=0):
        self.interpret_flag=True
        cond = captum.attr.LayerConductance(self, self.extra_layer)
        cond_vals = cond.attribute((test_dataset.x, test_dataset.additional), target=target_class)
        cols = self.extra_genes
        cond_vals_genomic = pd.DataFrame(cond_vals.detach().numpy(),
                                         columns=cols,
    ...
    def interpret(self, test_dataset, plot=False):
        ...
        extra_importances = self.extra_layer_importance(test_dataset)
        ...
        self.interpret_flag=False
        return gene_feature_importances, additional_feature_importances, gene_importances, layer_importance_scores, extra_importances
...
def evaluate_interpret_save(model, test_dataset, path):
    ...
    gene_feature_importances, additional_feature_importances, gene_importances, layer_importance_scores, extra_importances = model.interpret(test_dataset)
    ...
    extra_importances.to_csv(path+'/extra_importances.csv')
...
def run(genetic_data, target, save_path='../results/model', gene_set=None, additional_data=None, test_split=0.2, seed=None, dropout=0.2,
    ...
    reactome_network = ReactomeNetwork_2Regulatory.ReactomeNetwork(train_dataset.get_genes())
