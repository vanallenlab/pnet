...
class SankeyDiag:
    ...
    def gene_feature_to_gene_layer(self):
        ...
        for ind, elem in gene_feature_imps.nlargest(10, 'Importance').iterrows():
            ...
            if target not in list(gene_imps.nlargest(10, 'Importance')['Gene/Pathway']):
                target = 'Residual_1'
        ...
        target = 'Residual_1'
        source = 'Residual_0'
        value = gene_imps.nsmallest(gene_feature_imps.shape[0]-10, 'Importance')['Importance'].sum()/10
        self.append_links(self.numerical_encoding[source], self.numerical_encoding[target], value, RES_COLOR)
