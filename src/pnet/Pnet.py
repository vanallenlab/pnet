...
class Regulatory_Block(nn.Module):
    def __init__(self, regulatory_mask, activation='tanh', batchnorm=False, gene_dropout=None, dropout=None):
        ...
        self.regulatory_layer = nn.Sequential(*CustomizedLinear.masked_activation(regulatory_mask, activation=activation, batchnorm=batchnorm, dropout=gene_dropout))
...
class PNET_NN(pl.LightningModule):
    def __init__(self, reactome_network, task, nbr_gene_inputs=1, output_dim=1, additional_dims=0, lr=1e-3, weight_decay=1e-5,
        ...
        self.input_layer = nn.Sequential(*CustomizedLinear.masked_activation(input_mask, activation=self.activation, batchnorm=True, dropout=self.input_dropout))
        ...
        self.first_gene_layer = nn.Sequential(*CustomizedLinear.masked_activation(gene_masks[0], activation=self.activation, batchnorm=True, dropout=self.gene_dropout))
